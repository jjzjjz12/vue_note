...
1. 第一步：在`components`下面创建`index.ts`
...
3. 第二步：在`main.ts`中安装插件

4. ```ts
   // 引入自定义插件对象：注册整个项目的全局组件
   // 这个地方可能会报错在vite-enc.d.ts声明模块
...
5. ```ts
   ...
   **这样就不用在`main.ts`中使用大量`import xxx from xxx`和`app.component(xxx,xxx)`来定义全局组件了**
...
在src/styles目录下创建一个index.scss文件，当然项目中需要用到清除默认样式，也创建`reset.scss`,此文件代码从`npm`官网复制，因此在index.scss引入
